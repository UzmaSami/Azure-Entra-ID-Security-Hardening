...
│          Blocks all legacy auth protocols
│          SMTP, IMAP, POP3, basic auth
│
├── CA-003: Block High Risk Sign-ins
│          Identity Protection integration
│          Blocks sign-ins scored High risk
│
├── CA-004: Require Compliant Device
│          Report-only mode initially
│          Enforced after device compliance
│          baseline established
│
└── CA-005: Admin MFA Always
           All admin roles
           Every session — no exceptions

PRIVILEGED IDENTITY MANAGEMENT
├── Global Administrator
│   └── JIT only — 8hr max — approval required
├── Security Administrator
│   └── JIT only — 4hr max — no approval
├── User Administrator
│   └── JIT only — 4hr max — no approval
└── All other privileged roles
    └── JIT only — configurable per role
...
├── User Risk Policy
│   ├── High risk → Require password reset
│   └── Medium risk → Require MFA
└── MFA Registration Policy
    └── All users → Register within 14 days

ACCESS REVIEWS
├── Global Admin review — quarterly
├── Guest user review — monthly
└── Privileged role review — quarterly

HYBRID IDENTITY INTEGRATION
└── On-premises AD (UzmaSamiDC01)
    └── Synced via Azure AD Connect
        └── Same CA policies apply
            to synced identities
